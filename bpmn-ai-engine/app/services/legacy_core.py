...
offline_deep_learning_service = OfflineDeepLearningService()
predictive_orchestrator = PredictiveOrchestrator(offline_deep_learning_service)
...
def _predict_routes(tasks: List[PredictiveTaskEvent]) -> List[Dict[str, Any]]:
    ...
    transitions: Dict[tuple[str, str], Dict[str, int]] = {}
    for process_tasks in by_process.values():
        ordered = sorted(process_tasks, key=lambda task: task.createdAt or "")
        for left, right in zip(ordered, ordered[1:]):
            policy_id = left.policyId or right.policyId or ""
            bucket = transitions.setdefault((policy_id, left.taskId), {})
            bucket[right.taskId] = bucket.get(right.taskId, 0) + 1

    predictions: List[Dict[str, Any]] = []
    for (policy_id, current_task), next_counts in transitions.items():
        ...
        predictions.append(
            {
                ...
                "reason": "Predicción basada en las rutas que más se repiten en trámites similares.",
            }
        )
...
def predictive_analysis(payload: PredictiveAnalysisRequest, http_request: Request) -> PredictiveAnalysisResponse:
    ...
    result = predictive_orchestrator.analyze(
        payload,
        tensorflow_available=TENSORFLOW_AVAILABLE,
        autoencoder_anomalies=_autoencoder_anomalies,
        document_burst_anomalies=_document_burst_anomalies,
        predict_priorities=_predict_priorities,
        predict_routes=_predict_routes,
        predict_bottlenecks=_predict_bottlenecks,
    )

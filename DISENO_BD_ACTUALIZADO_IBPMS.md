# Diseño de Base de Datos Actualizado (Modelo Lógico y Físico)
...
Colección: `document_permissions`
  - `id`
  - `documentId`
  - `userId`
  - `username`
...
db.areas.createIndex({ company: 1 });
...
db.policies.createIndex({ companyId: 1, name: 1 });
...
db.task_instances.createIndex({ laneId: 1 });
db.task_instances.createIndex({ laneId: 1, status: 1 });
db.task_instances.createIndex({ processInstanceId: 1 });
db.task_instances.createIndex({ processInstanceId: 1, status: 1 });
db.task_instances.createIndex({ processInstanceId: 1, laneId: 1 });
db.task_instances.createIndex({ processInstanceId: 1, assignedTo: 1 });
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        documentCode: { bsonType: ["string", "null"] },
...
db.documents.createIndex({ processInstanceId: 1, createdAt: -1 });
db.documents.createIndex({ policyId: 1, createdAt: -1 });
db.documents.createIndex({ clientId: 1 });
db.documents.createIndex({ s3Key: 1 }, { unique: true });
...
db.createCollection("document_permissions", {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      required: ["documentId", "userId", "username", "company", "canView", "canEdit", "canDelete", "updatedAt"],
      properties: {
...
db.document_permissions.createIndex(
  { documentId: 1, userId: 1 },
  { unique: true, name: "document_user_unique" }
);
db.document_permissions.createIndex({ documentId: 1, username: 1 });
db.document_permissions.createIndex({ company: 1, userId: 1 });

// ==========================================
// Colección: document_audit_logs
// ==========================================
db.createCollection("document_audit_logs", {
  validator: {
    $jsonSchema: {
      bsonType: "object",
      required: ["username", "action", "documentId", "timestamp"],
      properties: {
        username: { bsonType: "string" },
        userId: { bsonType: ["string", "null"] },
        action: { enum: ["VIEW", "EDIT", "DELETE"] },
        documentId: { bsonType: "string" },
        processInstanceId: { bsonType: ["string", "null"] },
        httpMethod: { bsonType: ["string", "null"] },
...
db.document_audit_logs.createIndex({ documentId: 1, timestamp: -1 });
db.document_audit_logs.createIndex({ processInstanceId: 1, timestamp: -1 });
db.document_audit_logs.createIndex({ userId: 1, timestamp: -1 });
db.document_audit_logs.createIndex({ action: 1, timestamp: -1 });
